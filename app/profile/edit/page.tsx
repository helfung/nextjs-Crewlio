...
function ToggleGroup({ title, items, selected, setSelected, otherValue, setOtherValue }: {
  title: string; items: string[]; selected: string[]; setSelected: (v: string[]) => void;
  otherValue?: string; setOtherValue?: (v: string) => void;
}) {
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {items.map((item) => (
          <button key={item} onClick={() => toggle(item)}
            className={`rounded-full px-3 py-1 text-xs font-medium transition ${selected.includes(item) ? "bg-teal-700 text-white" : "bg-slate-100 text-slate-700"}`}>
            {item}
          </button>
        ))}
      </div>
...
export default function ProfileEditPage() {
  ...
  useEffect(() => {
    async function fetchProfile() {
      ...
      if (staffProfile) {
        setQualifications(staffProfile.qualifications || []);
        const knownSoftware = (staffProfile.skills || []).filter((s: string) => SOFTWARE.includes(s));
        const otherSkills = (staffProfile.skills || []).filter((s: string) => !SOFTWARE.includes(s));
        setSkills(otherSkills);
        setSoftware(knownSoftware);
        setAvailability(staffProfile.available_days || []);
        const hospAccred = (staffProfile.credentialled_hospitals || []).join(", ");
        if (hospAccred) {
          setHospitalAccreditations(hospAccred);
          setCertificates([...(staffProfile.qualifications || []), "Hospital Accreditation"]);
        }
      }
      setFetching(false);
    }
  ...
  async function handleSave() {
    ...
    const { error: staffError } = await supabase.from("staff_profiles").upsert({
      ...
      credentialled_hospitals: hospitalAccreditations ? hospitalAccreditations.split(",").map(h => h.trim()) : [],
    });
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-teal-50 p-4 md:p-8">
      <div className="mx-auto max-w-xl">
        <div className="rounded-3xl border border-slate-100 bg-white shadow-sm p-8">
          ...
          <h2 className="text-xl font-bold mt-8">Edit your profile</h2>
          <p className="text-sm text-slate-500 mt-1 mb-6">Keep your details up to date for better shift matches.</p>
          ...
          <div className="space-y-6">
            ...
            {saved && <p className="text-sm text-emerald-700 bg-emerald-50 rounded-2xl p-3">✓ Profile saved! Redirecting...</p>}
